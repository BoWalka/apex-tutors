...
# 1. Create the TutorChat component
CHAT_COMPONENT_CODE = """"use client";
...
with open("src/components/TutorChat.tsx", "w", encoding="utf-8") as f:
    f.write(CHAT_COMPONENT_CODE)
print("✓ Created src/components/TutorChat.tsx")

# 2. Update page.tsx to include the new component
page_path = "src/app/page.tsx"
with open(page_path, "r", encoding="utf-8") as f:
    page_content = f.read()

if "TutorChat" not in page_content:
    # Add the import
    page_content = page_content.replace(
        'import ContactForm from "@/components/ContactForm";',
        'import ContactForm from "@/components/ContactForm";\nimport TutorChat from "@/components/TutorChat";'
    )
    # Inject the component right before the Contact Form
    page_content = page_content.replace(
        '<ContactForm />',
        '<TutorChat />\n      <ContactForm />'
    )
    ...
    print("✓ Updated src/app/page.tsx to include TutorChat")
else:
    print("! TutorChat already exists in page.tsx")

print("\nDone! Run 'git add .', 'git commit -m \"Add AI Tutor Chat\"', and 'git push origin main' to deploy.")
